 ...
 *
 * It illustrates the use of the fuse_lowlevel_notify_expire_entry()
 * function.
 *
 * To see the effect, first start the file system with the
 * ``--no-notify``
 *
 *     $ notify_expire_entry --update-interval=1 --timeout 30 --no-notify mnt/
 *
 ...
 * ## Compilation ##
 *
 *     gcc -Wall notify_expire_entry.c `pkg-config fuse3 --cflags --libs` -o notify_expire_entry
 *
 * ## Source code ##
 * \include notify_expire_entry.c
 */
